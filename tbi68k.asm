...
;	2022/10/21	- Modified by Satoshi Okue
;
; ROM address 0x0000-0x3fff
; RAM address 0x8000-
;
...
ENDMEM	DC.L	ENDRAM		; end of available memory
;
...
CSTART	MOVE.L	ENDMEM,SP	; initialize stack pointer
	...
	SUBI.L	#2048,D0	; reserve 2K for the stack
	MOVE.L	D0,STKLMT
	SUBI.L	#108,D0		; reserve variable area (27 long words)
	MOVE.L	D0,VARBGN
	MOVE.L	#START,RANPNT
...
ST3	MOVE.B	#'>',D0		; Prompt with a '>' and
	...
	CMPI.L	#$FFFF,D1	; see if line no. is <= 16 bits
...
ST4	MOVE.L	A4,D0		; calculate the length of new line
	SUB.L	A0,D0
	CMPI.L	#3,D0		; is it just a line no. & CR?
...
EXNGO	MOVE.B	D0,D3		; Else check for period...
	...
	ANDI.B	#$7F,D1		; ignore the table's high bit
...
LS1	BCS	WSTART		; warm start if we passed the end
	...
	BEQ	LS3
	CMPI.B	#CTRLS,D0	; pause the listing?
	BNE	LS3
...
LOD1	BSR	GOAUXI		; look for start of line
	BEQ	LOD1
	CMPI.B	#'@',D0		; end of program?
	BEQ	LODEND
	CMPI.B	#':',D0		; if not, is it start of line?
...
LOD2	BSR	GOAUXI		; get another text char.
	...
	CMPI.B	#CR,D0		; is it the end of the line?
...
GBYTE2	ANDI.B	#$F,D0		; strip ASCII
...
PBYTE1	ROL.B	#4,D1		; get the next nybble
	MOVE.B	D1,D0
	ANDI.B	#$F,D0		; strip off garbage
	ADDI.B	#'0',D0		; make it into ASCII
	CMPI.B	#'9',D0
	BLS	PBYTE2
...
TSTV	BSR.L	IGNBLK
	...
	SUB.L	D1,D0		; into D0
	RTS
TV1	CMPI.B	#27,D0		; if not @, is it A through Z?
	EOR	#1,CCR
...
MLT1	TST.L	D1		; take absolute value of D1
	BPL	MLT2
	NEG.L	D1
MLT2	CMPI.L	#$FFFF,D1	; is second argument <= 16 bits?
	BLS	MLT3		; OK, let it through
	EXG	D0,D1		; else swap the two arguments
	CMPI.L	#$FFFF,D1	; and check 2nd argument again
...
ERROR	BSR.L	PRMESG		; display the error message
	...
	CMPI.L	#-1,D0		; is the line no. pointer = -1?
...
GL1	BSR.L	CHKIO		; check keyboard
	...
	CMPI.B	#CTRLH,D0	; delete last character?
	BEQ	GL3		; if so
	CMPI.B	#CTRLX,D0	; delete the whole line?
	BEQ	GL4		; if so
	CMPI.B	#CR,D0		; accept a CR
	BEQ	GL2
	CMPI.B	#' ',D0		; if other control char., discard it
	BCS	GL1
GL2	MOVE.B	D0,(A0)+	; save the char.
	BSR	GOOUT		; echo the char back out
	CMPI.B	#CR,D0		; if it's a CR, end the line
	BEQ	GL7
...
GL4	MOVE.L	A0,D1		; delete the whole line
	SUBI.L	#BUFFER,D1	; figure out how many backspaces we need
...
FNDLN	CMPI.L	#$FFFF,D1	; line no. must be < 65535
	BCC	QHOW
...
QT1	MOVE.L	A0,A1
	...
	CMPI.B	#LF,D0		; was last one a CR?
...
PN1	DIVU	#10,D1		; get the next digit
	...
	ANDI.L	#$FFFF,D1	; strip the remainder
...
TN1	CMPI.B	#'0',(A0)	; is it less than zero?
	...
	CMPI.L	#214748364,D1	; see if there's room for new digit
...
TOUPB1	MOVE.B	(A0)+,D0	; get the next text char.
	CMPI.B	#CR,D0		; is it end of line?
	BEQ	TOUPBRT		; if so, return
	CMPI.B	#'"',D0		; a double quote?
	BEQ	DOQUO
	CMPI.B	#"'",D0		; or a single quote?
	BEQ	DOQUO
...
;
TOUPPER	CMPI.B	#'a',D0		; is it < 'a'?
	BCS	TOUPRET
	CMPI.B	#'z',D0		; or > 'z'?
	BHI	TOUPRET
	SUBI.B	#32,D0		; if not, make it upper case
TOUPRET	RTS
...
CHKIO	BSR.L	GOIN		; get input if possible
	...
	CMPI.B	#CTRLC,D0	; is it control-C?
	BNE	CHKRET		; if not
...
;
OUTC	BTST	#1,ACIAC	; is port 1 ready for a character?
	BEQ	OUTC		; if not, wait for it
	MOVE.B	D0,ACIAD	; out it goes.
	RTS
...
;
INC	BTST	#0,ACIAC	; is character ready?
	BEQ	INCRET		; if not, return Zero status
	MOVE.B	ACIAD,D0	; else get the character
	ANDI.B	#$7F,D0		; zero out the high bit
INCRET	RTS
